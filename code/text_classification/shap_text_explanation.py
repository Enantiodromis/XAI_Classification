...
# SHAP EXPLAINER #
##################
def shap_explainer(X_test, y_test, model, word_index, save_path, tokenizer, max_sequence_length):
    ...
    """print(" ".join(my_texts))
    X_test_peturbed_words = [list(find_synonyms_and_perturb(X_data)) for X_data in my_texts[:num_explanations]]

    X_test_peturbed_encoded = []
    for lists in X_test_peturbed_words:
        # Converting string to explain back to a tokenized sequence. 
        cnn_rep = tokenizer.texts_to_sequences([lists])
        # Applying the original padding to the 
        X_test_peturbed_encoded.append(list(pad_sequences(cnn_rep, maxlen=max_sequence_length)))
    ...
    for index in range(num_explanations):
        shap.initjs()
        shap.save_html(save_path+'explanation_'+str(index)+'_'+str(class_names[y_test[index][0]])+'.html', shap.force_plot(explainer.expected_value[0], shap_values[0][index], my_texts[index]))
        print(" ".join(my_texts[index]))
        print(model.predict(X_test[index]))
        """shap.initjs()
        shap.save_html(save_path+'explanation_peturbed'+str(index)+'_'+str(class_names[y_test[index][0]])+'.html', shap.force_plot(explainer.expected_value[0], shap_values_peturbed[0][index], X_test_peturbed_words[index]))
        print(model.predict(X_test_peturbed_encoded))"""
...
shap_explainer(X_test, y_test, model, word_index, save_path, tokenizer, max_sequence_length) # Generating a explanation."""
