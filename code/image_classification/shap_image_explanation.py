...
def extracting_shap_explainer_explanation(train_generator, test_generator, model): 
    ...
    background = X_train[np.random.choice(X_train.shape[0], 3, replace=False)]
...
#history, model = img_classification_model(train_generator, test_generator, 50, model_name) # Training the model
...
train_generator, test_generator = binary_dataset_creation(4, 256, 256, False, True, file_path=folder_path) 
